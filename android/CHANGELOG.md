...
### Added (rc.4 — visual polish: neon glow, animated values, sparklines, G-force plot, shift lights)
- **Neon glow effects on hero values** — `HeroCard` now renders a soft radial gradient (`drawBehind` with `Brush.radialGradient`) behind the Orbitron value in each card's accent color (warm yellow for boost, orange for RPM, cyan for speed) at 18% alpha. Creates a subtle bloom/halo behind the number without requiring `RenderEffect` (works on all API levels ≥ 28). Same treatment applied to `GfCard` values at 12% alpha using the theme accent color.
- **Neon glow on progress bars** — `BarCard` gains optional `barGlowColor` parameter. When set, the progress bar fill renders a vertical `neonGlowRect` modifier (20% alpha, 4dp spread) that creates a soft light bleed above and below the 4dp bar. Applied to THROTTLE (cyan), BRAKE (orange), FUEL (green), and BATTERY (gold) bars on the DASH tab.
- **Connection dot glow ring** — the 6dp connection status dot in `AppHeader` now has a 14dp semi-transparent backing circle (25% alpha, synced to the existing pulse animation) creating a soft glow ring. Same treatment applied to the RTR "RACE READY" banner dot on the TEMPS tab (18dp ring at 20% alpha).
- **Active tab indicator glow** — the 2dp accent underline in `TabBar` now has a 6dp vertical gradient glow (12% alpha fading to transparent) beneath it, adding subtle depth to the selected tab.
- **Animated value transitions** — all numeric displays on the DASH tab now use `animateFloatAsState` with spring physics instead of snapping between values. Fast-changing values (RPM, speed, throttle, brake) use `Spring.StiffnessHigh` (10000f) for tight CAN bus tracking. Medium values (boost) use `Spring.StiffnessMediumLow` for smooth sweeps. Slow values (fuel, battery) use `tween(800ms)`. G-force values on the CHASSIS tab also animated with `StiffnessHigh`. Sentinel values (-99.0, -1.0) bypass animation and display placeholder text directly.
- **Sparkline mini-charts in HeroCards** — `HeroCard` gains optional `sparklineData: List<Float>?` parameter. When populated, a 22dp Canvas sparkline renders below the peak indicator showing ~15 seconds of value history. Line drawn with `StrokeCap.Round` at 1.5dp in the card's accent color (80% alpha), with a vertical gradient fill below (20% alpha fading to transparent). Data sampled at ~4 Hz via `SparklineData` ring buffer (60 samples) gated by a 250ms timestamp check during recomposition. Applied to BOOST, RPM, and SPEED hero cards on the DASH tab.
- **Sparkline mini-charts in BarCards** — `BarCard` gains optional `sparklineData: List<Float>?` parameter. 28dp sparkline renders below the progress bar. Applied to THROTTLE and BRAKE bars (fast-changing inputs). FUEL and BATTERY bars omit sparklines (too slow to produce meaningful trends).
- **G-Force dot plot replaces text cards on CHASSIS tab** — new `GForcePlot` Canvas composable (`ui/anim/GForcePlot.kt`) renders a full 2D G-force visualization: three concentric rings at 0.5G/1.0G/1.5G intervals with `Dim` stroke at 22% alpha, horizontal and vertical crosshair lines, axis labels (ACCEL/BRAKE/L/R), ring value labels at 45-degree offsets, and a live dot positioned by lateral × longitudinal G values. The dot renders with a 14dp radial glow halo (35% alpha), a solid 5dp accent-colored circle, and a 2dp white highlight. A comet trail of the last 30 positions (sampled at 10 Hz via `RingBuffer`) fades from 5% to 50% alpha oldest-to-newest, with 1.5dp connecting lines between consecutive points. The six `GfCard` text boxes are replaced by this plot plus two rows of compact `DataCell` readouts (LAT G, LON G, VERT G, YAW, STEER, COMBINED) preserving all numeric data. RESET PEAKS button retained.
- **RPM shift light bar on DASH tab** — new `ShiftLightBar` composable (`ui/anim/ShiftLightBar.kt`) renders 18 LED-style segments in a horizontal row between the hero cards and the gear indicator. Segments fill left-to-right proportional to RPM: green (Ok) for 0–4000 RPM, yellow (Warn) for 4000–5500, red (Orange) for 5500–6800. Each lit segment has a subtle vertical glow (`drawBehind` gradient at 20% alpha). At redline (≥6500 RPM, ~95.5% of 6800), all segments flash via `rememberInfiniteTransition` with a 120ms `LinearEasing` strobe between 100% and 15% alpha. Segments are 8dp tall with 3dp rounded corners and 1dp horizontal padding. The bar uses the animated RPM value from `animateFloatAsState` for smooth visual transitions.
- **Shared animation utilities** (`ui/anim/` subpackage):
  - `RingBuffer.kt` — generic fixed-capacity ring buffer with O(1) `push()` and `toList()` (oldest-first). Used by sparkline data and G-force trail.
  - `GlowModifiers.kt` — `Modifier.neonGlow(color, radius, alpha)` for radial glow and `Modifier.neonGlowRect(color, alpha, spread)` for rectangular bar glow. Both use `drawBehind` with gradient brushes (no `RenderEffect`, all API levels).
  - `Sparkline.kt` — `SparklineData` class (ring buffer wrapper with `push()`/`snapshot()`) and `Sparkline` Canvas composable that draws a polyline with gradient fill.
