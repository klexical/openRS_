...
### Added (rc.10.1 — TPMS tire temperature UI, Focus RS wireframe)
- **Tire temperature display in TPMS section** -- each `TireCard` now shows temperature below pressure (separated by a thin divider) when TPMS sensor temp data is available. Temperatures are color-coded: blue-grey (<15C cold), green (15-27C good), gold (28-40C warm), orange (>40C hot). Respects the global C/F temperature unit setting.
- **Focus RS MK3 wireframe replaces car outline placeholder** -- the old rectangle-and-dot `CarOutlinePlaceholder` is replaced with `FocusRsOutline`, a detailed top-down wireframe of the Focus RS rendered from a transparent PNG (`drawable-nodpi/focus_rs_wireframe.png`). Uses `ColorFilter.tint(accent, BlendMode.SrcIn)` so the wireframe automatically recolors to match whichever RS paint theme the user selects (Nitrous Blue, Race Red, Deep Orange, Stealth Grey, Shadow Black, Frozen White).
- **Temperature range legend** -- compact color-coded row (Cold / Good / Warm / Hot with colored dots) appears below the tire cards when temp data is present. Values adapt to the user's C/F unit preference.
- **`VehicleState.hasTireTempData`** -- new computed property; `true` when any tire temperature sensor has reported data (> -90C sentinel). Used to conditionally show the temp legend and update the section label to "PRESSURE & TEMPERATURE".
