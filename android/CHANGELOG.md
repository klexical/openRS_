...
### Fixed (rc.2)
- **Drive-mode dock cleanup on mid-flight dismiss** — `driveModePending` and `modeChangeBreath` now clear in a `finally {}` block so the header MODE pill pulse and bottom-nav indicator tint can't get stuck if the dock composition is disposed (external state change, backgrounding, rapid successor tap) before the `executeDriveModeChange` coroutine completes. Clears are also guarded with `if (modeChangeBreath.value == mode)` so a rapid successor tap doesn't wipe an incoming breath. (`ui/DriveModeDock.kt`)
- **BLE write deadlock on adapter disconnect** — `BleSlcanTransport.writeLine` previously blocked on `writeComplete.await()` forever when the adapter power-cycled or walked out of range mid-write, deadlocking `writeMutex` and freezing all subsequent OBD/firmware writes until app restart. `onConnectionStateChange(DISCONNECTED)` and `close()` now complete `writeComplete` + `connectDeferred` exceptionally; `writeCharacteristic` return value is checked; the `await()` is wrapped so failures surface to `SlcanConnection` retry logic. (`can/BleSlcanTransport.kt`)
- **DriveRecorder point loss on stop/crash** — `flushBuffer()` was fire-and-forget (`scope.launch(Dispatchers.IO) { dao.insertPoints(...) }`) after clearing the buffer synchronously, so up to 30 buffered points were lost when the scope was cancelled mid-write. `flushBuffer()` is now a suspend fn wrapped in `withContext(NonCancellable + Dispatchers.IO)` so the Room insert completes even from the recording loop's `finally {}`. `stopDrive()` now waits for the recorder job's finally flush (`jobToJoin?.join()`) before writing the drive summary, eliminating a race where both flush paths duplicated points. (`service/DriveRecorder.kt`)
- **Theme.kt stale font docstring** — comment claimed retired Barlow Condensed / Share Tech Mono TTFs still lived in `res/font/`; they are deleted in the "Daylight" pass. Updated to accurately describe the Kotlin alias retention. (`ui/Theme.kt`)

### Changed (rc.2 — header + drive mode)
- **Header geometry stabilised** — unified center-pill shape across all connection states (no more size/shape jump between "TAP TO CONNECT" and "RECONNECTING…"), fixed-width 3-char connection pill (LIVE/IDLE/OFF/RCN/UPD) that no longer reflows when its label hides, and a compact 34dp settings cog (was 44dp) with `Modifier.minimumInteractiveComponentSize()` preserving the 48dp touch target. (`ui/MainActivity.kt`, `ui/DesignTokens.kt`)
- **MODE + ESC always visible when connected** — the two pills are no longer gated behind the "Normal" header context; anomalies (e-brake, launch control, reconnecting, updating, offline) slide into a new 18dp anomaly strip directly below the header, leaving mode/ESC legible at the exact moments the driver cares most. (`ui/MainActivity.kt`)
- **Launch-control peripheral edge glow** — new `LaunchControlEdgeGlow` paints a warn-yellow frame at 700ms breath rhythm while `vs.launchControlEngaged` is true. Distinct cadence from `EdgeShiftLight` so the two cues never collide; still mirrored in the anomaly strip as "⚡ LAUNCH CONTROL". (`ui/anim/LaunchControlEdgeGlow.kt`)
- **Drive-mode dock is the single source of truth** — MorePage's interactive N/S/T/D grid was removed in favor of a display-only "CURRENT" readout + "CHANGE MODE ▾" button that opens the dock. Eliminates the duplicated tap handler that previously lived in both `MorePage.kt` and `DriveModeDock.kt`. (`ui/MorePage.kt`, `ui/GaragePage.kt`)
- **Pull-down gesture on the header** — vertical drag ≥ 40dp on the status bar opens the drive-mode dock from any tab. Complements the existing MODE-pill tap entry so discovery doesn't depend on the pill being in the Normal state. (`ui/MainActivity.kt`)
- **Cinematic drive-mode change** — pending button now draws a 15s circular progress arc with a mode-color shimmer sweep; on success an animated checkmark stroke reveals via `PathMeasure`, a mode-specific haptic fires (Normal: VirtualKey / Sport: Confirm / Track: Reject / Drift: GestureEnd), and an ambient mode-color breath briefly tints the header bottom underline + bottom-nav indicator before settling back to the accent. (`ui/DriveModeDock.kt`, `ui/BottomNavBar.kt`, `can/DriveCommand.kt`)
