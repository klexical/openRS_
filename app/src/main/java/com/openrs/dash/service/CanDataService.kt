...
class CanDataService : Service() {
        ...
        connectionJob = scope.launch {
            launch {
                wican.state.collect { state ->
                    ...
                    OpenOpenRSDashApp.instance.vehicleState.update {
            ...
            wican.connectHybrid(
                onObdUpdate = { obdState ->
                    ...
                    OpenOpenRSDashApp.instance.vehicleState.update { current ->
                        current.copy(
                            // Mode 1
                ...
                getCurrentState = { OpenOpenRSDashApp.instance.vehicleState.value }
            ).collect { (canId, data) ->
                val current = OpenOpenRSDashApp.instance.vehicleState.value
                val updated = CanDecoder.decode(canId, data, current)
                if (updated != null) {
                    OpenOpenRSDashApp.instance.vehicleState.value = updated
                        .withPeaksUpdated()
    ...
    fun stopConnection() {
        ...
        connectionJob = null
        OpenOpenRSDashApp.instance.vehicleState.update { it.copy(isConnected = false) }
        try { updateNotification("Disconnected") } catch (_: Exception) {}
    }

    fun resetPeaks() {
        OpenOpenRSDashApp.instance.vehicleState.update { it.withPeaksReset() }
    }
